...
example {n : ℤ} : 63 ∣ n ↔ 7 ∣ n ∧ 9 ∣ n := by
  ...
  calc
    ...
    _ = 4 * (9 * (7 * k)) - 5 * (7 * n) := by rw [h3]
    _ = 4 * (9 * (7 * k)) - 5 * (7 * (9 * l)) := by rw [h4]
    _ = 63 * (4 * k - 5 * l) := by ring
...
example {a b : ℤ} (hab : a ∣ b) : a ∣ 2 * b ^ 3 - b ^ 2 + 3 * b := by
  ...
  have h1 : 2 * b ^ 3 - b ^ 2 + 3 * b ≡ 0 [ZMOD a]:= by
    calc
